...
from .Span_based_encoder import Span_based_Encoder
from .NEC_decoder import NEC_Decoder
from .REL_decoder import REL_Decoder
...
class FWAS_Model(nn.Module):
    def __init__(self, config_for_model):
        super(FWAS_Model, self).__init__()
    ...
    def forward(self, data_list):
        ...
        output_dict = {}
        loss, NER_loss, RE_loss_1, RE_loss_2, logits_for_NER, answer_logits, batch_answer, class_logits, \
        ...
        output_dict["loss_RE_in_BF"] = RE_loss_1
        output_dict["loss_RE_in_RD"] = RE_loss_2
...
class Model_ALL(nn.Module):
    def __init__(self, config_for_model):
        ...
        self.encoder = Span_based_Encoder.from_pretrained(pretrained_model, encoder_config=config_for_model,
                                                          ner_label_count=num_ner_label)
        config = self.encoder.config
        self.decoder_for_BF = NEC_Decoder.from_pretrained(pretrained_model, decoder_config=config_for_model)
        self.decoder_for_RD = REL_Decoder(config, decoder_config=config_for_model)
    ...
    def forward(self, batch_tokens, batch_tokens_attention_mask, batch_spans, batch_span_labels, batch_span_mask,
        ...
        sequence_output_of_encoder_for_RD, rl_loss = self.encoder(batch_tokens, batch_tokens_attention_mask, batch_spans)
        ...
        loss = self.config_for_model.NER_loss_parameter * NER_loss + \
               ...
               self.config_for_model.RE_loss_for_BF_parameter * torch.sum(rl_loss)
